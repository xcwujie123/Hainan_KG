...
class Aggregator(object):
    ...
    def __call__(self, self_vectors, neighbor_vectors, neighbor_relations, neighbor_weights, user_embeddings):
        outputs = self._call(self_vectors, neighbor_vectors, neighbor_relations, neighbor_weights, user_embeddings)
        return outputs

    @abstractmethod
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, neighbor_weights, user_embeddings):
        # dimension:
        # self_vectors: [batch_size, -1, dim]
        # neighbor_vectors: [batch_size, -1, n_neighbor, dim]
        # neighbor_relations: [batch_size, -1, n_neighbor, dim]
        # neighbor_weights: [batch_size, -1, n_neighbor]
        # user_embeddings: [batch_size, dim]
        pass

    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, neighbor_weights, user_embeddings):
        avg = False
        if not avg:
            # [batch_size, 1, 1, dim]
            ...
            user_relation_scores=user_relation_scores*neighbor_weights
...
class SumAggregator(Aggregator):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, neighbor_weights, user_embeddings):
        # [batch_size, -1, dim]
        neighbors_agg = self._mix_neighbor_vectors(neighbor_vectors, neighbor_relations, neighbor_weights, user_embeddings)
