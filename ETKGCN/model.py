...
class KGCN(object):
    def __init__(self, args, n_user, n_entity, n_relation, adj_entity, adj_relation, adj_weight):
        self._parse_args(args, adj_entity, adj_relation, adj_weight)
        self._build_inputs()
    ...
    def _parse_args(self, args, adj_entity, adj_relation, adj_weight):
    ...
    def _build_model(self, n_user, n_entity, n_relation):
        ...
        entities, relations, weights = self.get_neighbors(self.item_indices)

        # [batch_size, dim]
        self.item_embeddings, self.aggregators = self.aggregate(entities, relations, weights)
    ...
    def get_neighbors(self, seeds):
        ...
        for i in range(self.n_iter):
            ...
            neighbor_weights=tf.reshape(tf.gather(self.adj_weight, entities[i]),[self.batch_size, -1])
            entities.append(neighbor_entities)
            relations.append(neighbor_relations)
            weights.append(neighbor_weights)
        return entities, relations, weights

    def aggregate(self, entities, relations, weights):
        ...
        for i in range(self.n_iter):
            ...
            #the shape of entity_vectors[hop]=[batch_size,n_neighbor**hop,dim]
            for hop in range(self.n_iter - i):
                shape = [self.batch_size, -1, self.n_neighbor, self.dim]
                shape_weight=[self.batch_size, -1, self.n_neighbor]
                vector = aggregator(self_vectors=entity_vectors[hop],
                                    neighbor_vectors=tf.reshape(entity_vectors[hop + 1], shape),
                                    neighbor_relations=tf.reshape(relation_vectors[hop], shape),
                                    neighbor_weights=tf.reshape(weights[hop],shape_weight),
