# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 15:14:44 2019
...
def load_data(args):
    ...
    n_entity, n_relation, adj_entity, adj_relation, adj_weight = load_kg(args)
    print('data loaded.')

    return n_user, n_item, n_entity, n_relation, train_data, eval_data, test_data, adj_entity, adj_relation, adj_weight
...
def load_kg(args):
    ...
    adj_entity, adj_relation, adj_weight = construct_adj(args, kg, n_entity)

    return n_entity, n_relation, adj_entity, adj_relation, adj_weight
...
def construct_adj(args, kg, entity_num):
    ...
    # each line of adj_weight store the corresponding sampled neighbor relations weight
    ...
    adj_weight= np.zeros([entity_num, args.neighbor_sample_size], dtype=np.float32)
    list_del=[]
    ...
    for entity in range(entity_num):
        ...
        adj_weight[entity]= np.array([arr_weight[neighbors[i][0]][entity] for i in sampled_indices])
    return adj_entity, adj_relation, adj_weight